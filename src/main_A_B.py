...
#%%
    ...
    for nb_B in  [10, 100, 20, 30, 40, 50, 60, 70, 80, 90]:
        ...
        dataset_non_conv.dict_co_expression = {'A': {'A': 1, 'B': -0.5138120546936563},
                                                'B': {'A': -0.5138120546936563, 'B': 1}}
       ...
       #     resize_mask = np.load("/media/tom/T7/simulation/exp_same_cyto/same_param1_4_0/remove20/resized_mask/08_IR5M_Pdgfra-Cy3_Serpine1-Cy5_010.tiff.npy")
        ...
        seq_scrna_centroids = np.load(
            f"/home/tom/Bureau/phd/simulation/mycode/centroid/scrna_centroidslist_cube0dico_simulation_A100_B100normalized_False.npy",
            allow_pickle=True)
        seq_scrna_unique_clusters = np.load(
            f'/home/tom/Bureau/phd/simulation/mycode/centroid/scrna_unique_clusterslist_cube0dico_simulation_A100_B100.npy',
            allow_pickle=True)
        seq_param_sctransform_anndata = np.load(
            f'/home/tom/Bureau/phd/simulation/mycode/centroid/param_sctransform_anndatalist_cube0dico_simulation_A100_B100.npy',
            allow_pickle=True)
        ...
        for image_name in list(dico_dico_commu.keys()):
            ...
            path_save = f"/media/tom/T7/regular_grid/simu1912/cube2D_step100/dataframe_folder/A_B/res/{date_str}/{nb_B}"
            Path(path_save).mkdir(parents=True, exist_ok=True)
            with open(f"{path_save}/" + 'result.txt', 'w') as f:
                f.write(
                  f"acc without taking into account no pred cell {round(acc, 4)}\n" +
                f"acc with taking into account no pred cell {round(acc_all_cell, 4)}\n" +
                f"% of predicted {len(y_true_cell_type_total) / len(y_true_cell_type_with_no_pred)}\n" +
                f"acc without taking into account no pred cell {round(acc, 4)}\n" +
                f"acc with taking into account no pred cell {round(acc_all_cell, 4)}\n" +
                f"% of predicted {len(y_true_cell_type_total) / len(y_true_cell_type_with_no_pred)}\n" +
                f"mean iou {round(np.mean(total_list_iou), 4)}\n" +
                f"mean prc_not_catch_list {round(np.mean(total_prc_not_catch_list), 4)}\n" +
                f"mean error_list {np.mean(total_error_list)}\n" +
                f"median iou {round(np.median(total_list_iou), 4)}\n" +
                f"median prc_not_catch_list {round(np.median(total_prc_not_catch_list), 4)}\n" +
                f"median error_list {round(np.median(total_error_list), 4)}\n" +
                  str(args)
                      )
            print()
            print(args)
            Comsegdict.save(Path(args.path_save) / (f"Comsegdict_nb_B{nb_B}." + date_str + "pickle.txt"))
            np.save(Path(args.path_save) / (f"args_nb_B{nb_B}." + date_str + ".npy"), args)
            print("model saved")
